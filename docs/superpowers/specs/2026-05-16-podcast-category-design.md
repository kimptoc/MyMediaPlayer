...
The space in `spoken word` matters — matching bare `spoken` would catch unrelated words like `Unspoken` or `Outspoken`. `Spoken Word` is also the ID3 v2 genre name (180), so the phrase match aligns with what tagged files actually carry.
