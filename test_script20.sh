...
class TempTest {
    ...
    fun extractCandidateMetadata_whenRetrieverThrowsException_andDoesNotRequireProbe_returnsFallback() = runBlocking {
        ...
        println("Suspend result: $suspendResult")
    }
}
EOF2
...
cat ./shared/build/reports/tests/testDebugUnitTest/classes/com.example.mymediaplayer.shared.TempTest.html | grep -i "Direct result" -A 2
